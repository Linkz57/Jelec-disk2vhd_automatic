...
function clean {
	...
	## grep also removes any avhdx files that Hyper-V uses for snapshots
	## grep finally removes the backup.log that my disk2vhd_automatic.bat keeps
	## tail is then given a list of non-folders sorted by age of last modification, and then ignores the 4 at the top (newest)
	## the output of tail (all except 4 newest) is saved in a text file
	ls -tp | grep -v '/$\|.avhdx\|backup.log' | tail -n +5 >> /mnt/disk2vhd/clean.log
	...
	ls -tp | grep -v '/$\|.avhdx\|backup.log' | tail -n +5 | xargs -d '\n' rm --
...
## an old VHD file that is currently being virtualized is being written to, meaning recent modification
